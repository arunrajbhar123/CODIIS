export const Login_User_Request = "Login_User_Request";
export const Login_User_Success = "Login_User_Success";
export const Login_User_Failure = "Login_User_Failure";

export const Signup_User_Request = "Signup_User_Request";
export const Signup_User_Success = "Signup_User_Success";
export const Signup_User_Failure = "Signup_User_Failure";

export const Session_User_Request = "Session_User_Request";
export const Session_User_Success = "Session_User_Success";
export const Session_User_Failure = "Session_User_Failure";

export const Video_Upload_Request = "Video_Upload_Request";
export const Video_Upload_Success = "Video_Upload_Success";
export const Video_Upload_Failure = "Video_Upload_Failure";

export const Logout_Success = "Logout_Success";

export const Get_Video_Request = "Get_Video_Request";
export const Get_Video_Success = "Get_Video_Success";
export const Get_Video_Failure = "Get_Video_Failure";

export const Get_Video_Single_Request = "Get_Video_Single_Request";
export const Get_Video_Single_Success = "Get_Video_Single_Success";
export const Get_Video_Single_Failure = "Get_Video_Single_Failure";

export const Get_Friends_Request = "Get_Friends_Request";
export const Get_Friends_Success = "Get_Friends_Success";
export const Get_Friends_Failure = "Get_Friends_Failure";


export const Post_Plan_Request = "Post_Plan_Request";
export const Post_Plan_Success = "Post_Plan_Success";
export const Post_Plan_Failure = "Post_Plan_Failure";

export const Update_Post_Plan_Request = "Update_Post_Plan_Request";
export const Update_Post_Plan_Success = "Update_Post_Plan_Success";
export const Update_Post_Plan_Failure = "Update_Post_Plan_Failure";

export const Get_Plan_Success = "Get_Plan_Request";

export const Delete_Post_Plan_Request = "Delete_Post_Plan_Request";
export const Delete_Post_Plan_Success = "Delete_Post_Plan_Success";
export const Delete_Post_Plan_Failure = "Delete_Post_Plan_Failure";